<section class="container py-5">
    <h6 class="text-uppercase text-left mx-auto">
        I progetti
    </h6>
    <h2 class="text-left mb-0 pb-2">
        <span class="text-primary">Cosa abbiamo fatto</span> – <span class="">e cosa stiamo facendo</span>.
    </h2>
    <h6 class="font-weight-300 mb-5">Progetti interattivi, ricerche, strumenti e giochi matematici realizzati dalla classe. In crescita ogni anno.</h6>


    <div class="row mx-0">

        <!-- esperienza di classe -->

        <div class="d-flex justify-content-center w-100 mb-4 mt-3 mt-md-2 position-relative">
            <hr class="border-top-dark position-center-center w-100 z-index-0 m-0">
            <div class="text-uppercase bg-white px-4 d-flex justify-content-center align-items-center position-relative mb-0"
                 style="height: 30px; z-index:1;">
                Esperienza di classe
            </div>
        </div>



        <!-- in evidenza -->
        <div class="col-12 d-flex rounded-lg border px-0 mb-4">
            <div class="w-md-50 p-4">
                <small class="text-success font-weight-500 d-flex align-items-center justify-content-start mb-2">
                    <span class="rounded-xsmall bg-success mr-3"></span>
                    <span class="">Interattivo</span>
                </small>
                <h3>Scomporre per comprendere</h3>
                <p class="text-secondary">Wizard guidato alla scomposizione dei polinomi. Con appunti PDF della
                    classe inclusi.</p>
                <div class="d-flex align-items-center justify-content-between mt-4">
                    <small class="bg-success-25 text-success px-3 rounded-pill mb-0">Wizard</small>
                    <a href="" title="Scomporre per comprendere | Sottoradice"><span
                            class="icon icon-arrow-right"></span></a>
                </div>
            </div>
            <picture class="w-md-50 bg-success-25">
                <img class="img object-cover" style="height: 230px;"
                     src="assets/images/sottoradice_scomporre_per_comprendere.png" alt="Scomporre per comprendere | Sottoradice">
            </picture>
        </div>

        <!-- altri progetti -->
        <div class="col-6 rounded-lg border p-4 mb-4 mb-lg-0 mr-lg-4" style="max-width: calc(50% - .75rem);">

            <small class="text-warning font-weight-500 d-flex align-items-center justify-content-start mb-2">
                <span class="rounded-xsmall bg-warning mr-3"></span>
                <span class="">Gioco</span>
            </small>
            <h3>Tombola matematica</h3>
            <p class="text-secondary">Il classico gioco reinventato: ogni numero nasconde un quesito. Estrai,
                risolvi, segna. Vince chi fa tombola! Modalità classica e sfida.</p>
            <div class="d-flex align-items-center justify-content-between mt-4">
                <small class="bg-warning-25 text-warning-plus px-3 rounded-pill mb-0">Giocabile online</small>
                <a href="" title="Tombola matematica | Sottoradice"><span class="icon icon-arrow-right"></span></a>
            </div>
        </div>


        <div class="col-6 rounded-lg border p-4 mb-0" style="max-width: calc(50% - .75rem);">
            <small class="text-pink font-weight-500 d-flex align-items-center justify-content-start mb-2">
                <span class="rounded-xsmall bg-pink mr-3"></span>
                <span class="">Ricerca</span>
            </small>
            <h3>La scuola pitagorica e √2</h3>
            <p class="text-secondary">
                Ricerca sulla nascita di √2 e la scuola pitagorica. I migliori PowerPoint realizzati dalla classe.
            </p>
            <div class="d-flex align-items-center justify-content-between mt-4">
                <small class="bg-pink-25 text-pink px-3 rounded-pill mb-0">Presentazioni PPT</small>
                <a href="" title="La scuola pitagorica e √2 | Sottoradice"><span
                        class="icon icon-arrow-right"></span></a>
            </div>
        </div>


        <!-- proposta docente -->

        <div class="d-flex justify-content-center w-100 mb-4 mt-5 position-relative">
            <hr class="border-top-dark position-center-center w-100 z-index-0 m-0">
            <div class="text-uppercase bg-white px-4 d-flex justify-content-center align-items-center position-relative mb-0"
                 style="height: 30px; z-index:1;">
                Proposta docente
            </div>
        </div>

        <div class="col-6 rounded-lg border mb-4 mb-lg-0 mr-lg-4 px-0" style="max-width: calc(50% - .75rem);">

            <picture class="w-md-50 bg-purple-25">
                <img class="img object-cover" style="height: 230px;"
                     src="assets/images/super_mario.jpg" alt="Super Mario e i vettori | Sottoradice">
            </picture>

            <div class="p-4">
                <small class="text-purple font-weight-500 d-flex align-items-center justify-content-start mb-2">
                    <span class="rounded-xsmall bg-purple mr-3"></span>
                    <span class="">GeoGebra Classic</span>
                </small>
                <h3>Super Mario e i vettori</h3>
                <p class="text-secondary">Geometria piana attraverso il gioco più amato. Creato dalla prof.ssa di matematica come strumento didattico.</p>
                <div class="d-flex align-items-center justify-content-between mt-4">
                    <small class="bg-purple-25 text-purple px-3 rounded-pill mb-0">Proposta docente</small>
                    <a href="" title="Tombola matematica | Sottoradice"><span class="icon icon-arrow-right"></span></a>
                </div>
            </div>
        </div>


        <div class="col-6 rounded-lg border mb-0 px-0" style="max-width: calc(50% - .75rem);">

            <picture class="w-md-50 bg-purple-25">
                <img class="img object-cover" style="height: 230px;"
                     src="assets/images/pacman.png" alt="Pac-Man 3D | Sottoradice">
            </picture>

            <div class="p-4">
                <small class="text-purple font-weight-500 d-flex align-items-center justify-content-start mb-2">
                    <span class="rounded-xsmall bg-purple mr-3"></span>
                    <span class="">GeoGebra 3D</span>
                </small>
                <h3>Pac-Man 3D</h3>
                <p class="text-secondary">
                    La sfera nello spazio cartesiano. Esercitazione interattiva creata dalla prof.ssa di matematica.
                </p>
                <div class="d-flex align-items-center justify-content-between mt-4">
                    <small class="bg-purple-25 text-purple px-3 rounded-pill mb-0">Proposta docente</small>
                    <a href="" title="La scuola pitagorica e √2 | Sottoradice"><span
                                class="icon icon-arrow-right"></span></a>
                </div>
            </div>
        </div>

    </div>
</section>